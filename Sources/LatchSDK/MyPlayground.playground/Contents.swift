...
do {
    ...
    //let latchResponse = try await latchSDK.pair(token: "tTiYvn")
    // App push: Servicio pareado
    // {"data":{"accountId":"c8pV6Ks8428MrUthf696EP4BcYmYedx9fnH9NXbMJPBYyxMjEWTihxZDzZjN2Mqz"}}
    
    // MARK: Pair a Latch application with ID
    let latchResponse = try await latchSDK.pairWithId("salavert2@gmail.com")
    
    // MARK: Pair a Latch application Web3
    //let latchResponse = try await latchSDK.pair(token: "NAecGP", web3Wallet: "any-wallet", web3Signature: "any-signature")
    
    // MARK: Unpair a Latch application
    //let latchResponse = try await latchSDK.unpair(accountId: accountId)
    // App push: Servicio despareado
    
    // MARK: Status of an application
    //let latchResponse = try await latchSDK.status(accountId: accountId)
    // App push: Acceso al servicio
    // {"data":{"operations":{"nRtj8te3quNZKWeCmJZq":{"status":"on"}}}}
    
    // MARK: Locks an application
    //let latchResponse = try await latchSDK.lock(accountId: accountId)
    // App push: Latch modificado
    
    // MARK: Unlocks an application
    //let latchResponse = try await latchSDK.unlock(accountId: accountId)
    // App push: Latch modificado
    
    // MARK: History
    //let latchResponse = try await latchSDK.history(accountId: accountId)
    // App push: Latch modificado
    
    // MARK: User Applications
    //let latchResponse = try await latchSDK.getUserApplications()
    ...
    //let latchResponse = try await latchSDK.addOperation(
    //    parentId: appId,
    //    details: .init(
    ...
    //let latchResponse = try await latchSDK.operation(operationId: "CKBcijpiMyCb6PvmAKnb")
    //{"data":{"operations":{"operationId":{"name":"...","two_factor":"...","lock_on_request":"...","operations":{...}}}}}
    
    
    //let latchResponse = try await latchSDK.modifyOperation(
    ...
    //let latchResponse = try await latchSDK.deleteOperation(operationId: "ftFnNiK2daN948hVA8U7")
    
    // MARK: User Applications
    
    //let latchResponse = try await latchSDK.addUserApplication(details: .init(
    //    name: "PlaygroundApp",
    ...
    //let latchResponse = try await latchSDK.modifyUserApplication(
    ...
    //let latchResponse = try await latchSDK.deleteUserApplication(
    //    appId: "y79q42qcm6WixFWBsnhq"
    //)
    
    //let latchResponse = try await latchSDK.subscription()
    
    print("\nRESPONSE BODY")
    print(String(data: latchResponse.rawData, encoding: String.Encoding.utf8)!)
